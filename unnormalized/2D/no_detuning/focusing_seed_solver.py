...
#Integrator Function -----------------------------------------------------------------
def solver_3(times_to_save = times_to_save, b_exp = b_exp):
    """
    2D practical solver...
    w/ seed focusing
    """
    ...
    ########################################################################################################################################
    # focused SEED: longitudinal Gaussian x radial Gaussian x lens phase (matches solver_1)
    ...
    for i in range(Ntau - 1):
        ...
        a_slice[0, :] = a0*np.exp(-(rad/p_sigma)**4)   # a[:,0,:] = a0  (flat pump entrance)
...
#Example usage
a, b, f, phi_a, phi_b, phi_f, b_axis, a_in_col, a_out_col = solver_3()
np.savez('TESTFILE.npz', 

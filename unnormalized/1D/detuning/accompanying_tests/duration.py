...
#Pulse Duration over time--------------------------------------------
time_vals_ns = np.arange(0, 130, 0.5)          # directly specify 0 to 130 ns
...
# pulse duration vs time, one curve per detuning run (auto-adjusts to nruns)
durs_t = [pulse_duration(b=b_runs[i])[t_idx] for i in range(nruns)]

colors = plt.cm.viridis(np.linspace(0, 1, nruns))
for i in range(nruns):
    if detuning_factors[i] == 0:
        plt.errorbar(times, durs_t[i], label="No Detuning Reference",
                     color="black", linestyle="--")
    else:
        plt.errorbar(times, durs_t[i], label=f"d = {detuning_factors[i]}", color=colors[i])

plt.xlabel("Time (ns)")
plt.ylabel("Pulse Duration")
plt.title("Seed Growth Pulse Duration (80% area) for Fixed Detuning")
plt.legend()

"""
With detuning. This is the full solver as described by the numerical methods
...
#Integrator Function -----------------------------------------------------------------
def full_2D_detuning_scheme(b_exp = 2, d = d):
    ...
    def advance_a_f_euler(i_idx_in, j_idx_in):
        ...
        f_new = (g*a_new*np.conj(b[i_idx_in,j_idx_in+1,:]) + kappa/dzeta * f[i_idx_in, j_idx_in,:]) / (1 - 1j*d + kappa/dzeta)
        return a_new, f_new
    ...
    def advance_a_f_RK4(i_idx_in, j_idx_in):
        ...
        for k in range(Nrad):
            if k == 0:
                ...
                A[k] = AK1[k]*amid2[k-1] + (1 - AK2[k])*amid2[k] + AK3[k]*amid2[k+1]
        a_new = solve_banded((1,1), AM, A)
        f_new = (g*a_new*np.conj(b[i_idx_in,j_idx_in+1,:]) + kappa/dzeta * ff) / (1 - 1j*d + kappa/dzeta)
        return a_new, f_new
    ########################################################################################################################################
    #####LOOP###############################################################################################################################
    ########################################################################################################################################

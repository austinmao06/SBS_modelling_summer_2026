"""
Mirror of focusing_seed_solver that turns off pump over some time
"""
...
#Integrator Function -----------------------------------------------------------------
def detuning_pump_off(times_to_save = times_to_save, d = d,
                      time_to_off = 127e-9, decrease_duration = 3e-9):
    """Mirror of detuning_1 with a TIMED pump shutoff at the left (entrance) boundary.

    The entrance pump column a[:,0] is held at full strength until `time_to_off` (s),
    then multiplied by a 4th-power super-gaussian that falls to ~0 over `decrease_duration`
    (s). The gate is one scalar per time step, so every radius drops by the SAME fraction
    (the radial pump profile shape is preserved -- fractional decrease equal over all r).

    Super-gaussian sigma = decrease_duration / 3, so `decrease_duration` = 3 sigma and the
    gate is exp(-81) ~ 0 there; it is clipped to exactly 0 afterwards (kills the denormal
    tail and matches the experimental 'pump fully off'). This is an AMPLITUDE gate on a,
    so the entrance intensity ~ gate**2. Everything else is identical to detuning_1.

    NOTE (fast-pump model): a is slaved to the instantaneous entrance BC (d_tau a dropped),
    so gating the entrance turns the pump off EVERYWHERE within the gate window, not via a
    shutoff front propagating in at c/n. That is consistent with this solver family.

    time_to_off       : lab time (s) at which the entrance pump begins to drop (default 126.7 ns).
    decrease_duration : time (s) over which it falls from full to 0 (default 1.3 ns).
    Returns the same 9 values as detuning_1.
    """
    tau  = np.arange(tau_range[0], tau_range[1], dtau)
    zeta = np.arange(zeta_range[0], zeta_range[1], dzeta)
    rad  = np.arange(rad_range[0], rad_range[1], drad)
    ...
    ##### TIMED PUMP GATE at the entrance (scalar per time step) #####
    sg_sigma  = decrease_duration / 3                                   # 3 sigma = full decrease_duration
    pump_gate = np.where(tau <= time_to_off, 1.0,
                         np.exp(-((tau - time_to_off)/sg_sigma)**4))       # 1 -> 4th-power super-gaussian
    pump_gate[tau >= time_to_off + decrease_duration] = 0.0               # clip tail to exactly 0
    ...
    ##### ADVANCE FUNCTIONS (identical to detuning_1) #####
    ...
    def zeta_march(b_slice, a_entrance):
        a_s = np.empty((Nzeta, Nrad), dtype=complex); f_s = np.empty((Nzeta, Nrad), dtype=complex)
        a_s[0, :] = a_entrance; f_s[0, :] = 0          # <-- gated pump entrance
    ...
    a_t = np.ones((Nzeta, Nrad), dtype=complex) * (pump_prof * pump_gate[0])[None, :]   # a[0] (gated, flat in zeta)
    ...
    for i in range(Ntau - 1):
        t = i + 1
        a_slice, f_slice = zeta_march(b_curr, pump_prof * pump_gate[t])  # gated entrance at tau[t]
        if t in pos:

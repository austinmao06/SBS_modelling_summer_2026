"""
With detuning. Viable solver with focusing seed + diffraction off. Good for running detuning runs in parallel. 
...
#Integrator Function -----------------------------------------------------------------
def diffraction_off_temp(times_to_save=times_to_save):
    """
    2D solver with the DIFFRACTION OPERATOR TURNED OFF.

    With diffraction off and radially-flat initial/boundary conditions
    (a = a0 everywhere, seed flat in r, f = 0), every radius decouples and
    evolves independently under coupling alone -> each radial column must
    reproduce the 1D solver exactly. This is validation-ladder step 3.

    To turn diffraction back ON: uncomment the CN blocks marked
    'DIFFRACTION OFF' below and re-comment the pass-through lines.
    """
    tau  = np.arange(tau_range[0], tau_range[1], dtau)
    zeta = np.arange(zeta_range[0], zeta_range[1], dzeta)
    rad  = np.arange(rad_range[0], rad_range[1], drad)
    ...
    ########################################################################################################################################
    #####MATRIX BANDS (unused while diffraction is off; kept so toggling back on is trivial)################################################
    ########################################################################################################################################
    ...
    ########################################################################################################################################
    #####ADVANCE FUNCTIONS (DIFFRACTION OFF)################################################################################################
    ########################################################################################################################################
    def advance_b(a_slice, f_slice, b_slice):
        """b[t] -> b[t+1]. Coupling only; both CN half-steps are the identity. All (Nzeta, Nrad)."""
        # ---- DIFFRACTION OFF: first half CN step is the identity ----
        ...
        # ---- DIFFRACTION OFF: second half CN step is the identity ----
        # for k in range(Nrad):
        #     if k == 0:
    ...
    def advance_a_f_euler(a_slice, f_slice, b_slice, j):
        """Advance pump/phonon one zeta step: (a,f)[t, j] -> (a,f)[t, j+1]. Coupling only."""
        # ---- DIFFRACTION OFF: first half CN step is the identity ----
        ...
        amid1 = a_slice[j, :]                      # NOTE: [j,:], not a_slice — that was a shape bug
    ...
    b_prev[:, :] = b0 * np.stack([np.exp(-np.abs(zeta/s)**b_exp) for s in np.linspace(sigma_range[0], sigma_range[1], Nrad)]).T
    a_slice = np.ones((Nzeta, Nrad), dtype=complex) #* (a0*np.exp(-(rad/p_sigma)**4))[None, :]          # a[0] (flat, undepleted)
    ...
    for i in range(Ntau - 1):
        ...
        a_slice[0, :] = a0#*np.exp(-(rad/p_sigma)**4)   # a[:,0,:] = a0  (left-boundary pump, flat)
        f_slice[0, :] = 0    # f[:,0,:] = 0   (no acoustic at left)
        for j in range(Nzeta - 1):
            if 2*G > 1:  # stiff condition (placeholder)
    ...
    return (np.abs(a_out), np.abs(b_out), np.abs(f_out),
            np.angle(a_out), np.angle(b_out), np.angle(f_out),
            b_axis, a_in_col, a_out_col)
